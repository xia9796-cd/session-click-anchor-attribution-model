...
## 目的
`official-events`ページごとのユーザー価値やアクション数を測るため、
`official-events`ページ以下の特定のページまでを、`official-events`ページを起点としたアトリビューション分析基盤を作成する。
...
```
この動きでユーザーが回遊する。
この一連の動きで起きる各ページのセッションとクリックイベント、そのイベントのCTRを、入り口ページである`official-events`ページに
のせる分析基盤を作成する。
